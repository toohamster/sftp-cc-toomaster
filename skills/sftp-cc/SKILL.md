...
- "部署代码"、"把文件传到服务器上"
- "sftp 上传"、"sftp 同步"
- "サーバーに同期する", "サーバーにアップロードする", "デプロイする"
...
**注意**：`${CLAUDE_PLUGIN_ROOT}` 是 Claude Code 注入的 Skill 内部变量，仅在 Skill 上下文中有效。执行脚本时，Claude 会自动将其解析为插件根目录路径（如 `~/.claude/plugins/marketplaces/sftp-cc/`）。
...
# 上传指定文件
bash ${CLAUDE_PLUGIN_ROOT}/scripts/sftp-push.sh src/index.php config.php
...
# 详细输出
bash ${CLAUDE_PLUGIN_ROOT}/scripts/sftp-push.sh -v
```
...
| 用户指令 | 执行脚本 |
|---------|---------|
| "同步代码到服务器"、"上传文件"、"部署代码" | `sftp-push.sh` |
| "绑定私钥"、"绑定 SSH 密钥" | `sftp-keybind.sh` |
| "初始化配置"、"配置服务器" | `sftp-init.sh` |
| "部署公钥"、"配置 SSH 免密" | `sftp-copy-id.sh` |

## 操作注意事项

- 每次上传前会自动运行 keybind 检查私钥状态
